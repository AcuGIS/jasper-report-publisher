L.Control.Opacity = L.Control.extend({
    ...
    _addItem: function (obj) {
        ...
        if (obj.overlay) {
            ...
            input.min = 0;
            input.max = 100;
						if (typeof obj.layer.getLayers === 'function') {
							sublayers = obj.layer.getLayers();
							//obj.layer.eachLayer(function (sublayer) {
	            //	input.value = sublayer.options.opacity * 100;
	            //});
							input.value = sublayers[0].options.opacity * 100;
						}else{
        ...
        input.addEventListener('input', (event) => {
            const rgValue = event.target.value / 100;
            const layer = this._getLayer(input.layerId).layer;
            // 背景ラスタのみ対象
            if (typeof obj.layer.getLayers === 'function') {
							var style = {"opacity": rgValue, "fillOpacity": rgValue};
							
							layer.eachLayer(function (sublayer) {
								if(sublayer.setStyle){
									sublayer.setStyle(style);
								}else{	// for L.Geoserver.wms() layers
									sublayer.setOpacity(rgValue);
								}
								layer.options.opacity = rgValue;
								layer.options.fillOpacity = rgValue;
							});
            } else {
                layer.setOpacity(Number(rgValue));
            }
        });
