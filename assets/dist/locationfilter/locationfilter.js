...
L.LocationFilter = L.Layer.extend({
    ...
    onAdd: function(map) {
        ...
        if (this.options.enableButton || this.options.pdfButton) {
    ...
    onRemove: function(map) {
		...
		getExtent: function(){
			var crs4326 = '+proj=longlat +datum=WGS84 +no_defs';
			var crs3857 = '+proj=merc +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +no_defs';
			var proj4326 = proj4(crs4326);
			var proj3857 = proj4(crs3857);

			let bounds = this.getBounds();
			// Convert latitude and longitude to EPSG:3031
			var projectedCoordsMin = proj4(proj4326, proj3857, [bounds.getSouthWest().lng, bounds.getSouthWest().lat]);
			var projectedCoordsMax = proj4(proj4326, proj3857, [bounds.getNorthEast().lng, bounds.getNorthEast().lat]);
			
			var minx = projectedCoordsMin[0];
			var miny = projectedCoordsMin[1];
			var maxx = projectedCoordsMax[0];
			var maxy = projectedCoordsMax[1];
    ...
    enable: function() {
        ...
        if (this.options.pdfButton) {
            this._createPDFButton();
        }
				
				if (this.options.geopdfButton) {
						this._createGeoPDFButton();
				}
				
				if (this.options.pngButton) {
            this._createPNGButton();
        }
    ...
    disable: function() {
        ...
        if (this._pdfButton) {
            this._pdfButton.remove();
        }
				
				if (this._geopdfButton) {
            this._geopdfButton.remove();
        }
				
				if (this._pngButton) {
            this._pngButton.remove();
        }
    ...
    _createPDFButton: function() {
        var that = this;
        this._pdfButton = new L.Control.Button({
            className: "adjust-button",
            text: this.options.pdfButton.text,
            
            onClick: function(event) {
								let print_url = 'proxy_qgis.php?SERVICE=WMS&REQUEST=GetPrint&CRS=EPSG:4326&FORMAT=pdf&CRS=EPSG:3857&TEMPLATE=' + that.options.qgisTemplate + '&map0:EXTENT=' + that.getExtent();
								window.open(print_url, '_blank');
            }
        }).addTo(this._buttonContainer);
    },
		
		_createGeoPDFButton: function() {
        var that = this;
        this._geopdfButton = new L.Control.Button({
            className: "adjust-button",
            text: this.options.geopdfButton.text,
            
            onClick: function(event) {
								let print_url = 'proxy_qgis.php?SERVICE=WMS&REQUEST=GetPrint&CRS=EPSG:4326&FORMAT=pdf&FORMAT_OPTIONS=WRITE_GEO_PDF:TRUE;APPEND_GEOREFERENCE=TRUE&CRS=EPSG:3857&TEMPLATE=' + that.options.qgisTemplate + '&map0:EXTENT=' + that.getExtent();
								window.open(print_url, '_blank');
            }
        }).addTo(this._buttonContainer);
    },
		
		_createPNGButton: function() {
        var that = this;
        this._pngButton = new L.Control.Button({
            className: "adjust-button",
            text: this.options.pngButton.text,
            
            onClick: function(event) {
								let print_url = 'proxy_qgis.php?SERVICE=WMS&REQUEST=GetPrint&CRS=EPSG:4326&FORMAT=png&CRS=EPSG:3857&TEMPLATE=' + that.options.qgisTemplate + '&map0:EXTENT=' + that.getExtent();
								window.open(print_url, '_blank');
            }
        }).addTo(this._buttonContainer);
    },
